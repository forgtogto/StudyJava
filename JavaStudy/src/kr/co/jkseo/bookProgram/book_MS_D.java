...
public abstract class book_MS_D { // 도서 및 회원 관리기능 클래스
	...
	public static void bookSave_DB(String str1, String str2, String str3, String str4) { // DB에 도서저장하는 메소드
		...
		try {
			...
			conn = DriverManager.getConnection("jdbc:mysql://localhost:3306/book", "root", "qwer");
			stmt = conn.createStatement();
			ResultSet rs = stmt.executeQuery("select code from booksinfo where code = '" + str4 + "';");
			if (rs.next()) {
				int rowNum = stmt.executeUpdate("update booksinfo set name:='" + str1 + "', writer:='" + str2
						+ "', publisher:='" + str3 + "' where code='" + str4 + "';");
				...
				String queryLang = "insert into booksinfo (name, writer, publisher, code, forrent) values('" + str1
						+ "', '" + str2 + "', '" + str3 + "', '" + str4 + "', '가능');";
	...
	public static void memberSave_DB(String memberNameStr, String addressStr, String phoneStr) { // DB에 회원정보 저장하는 메소드
		...
		try {
			...
			stmt = conn.createStatement();
			ResultSet rs = stmt
					.executeQuery("select name, address, phone from membersinfo where phone = '" + phoneStr + "';");
			String name = null;
			String address = null;
			String phone = null;
			while (rs.next()) {
			...
			if (!(name == null)) {
				stmt.executeUpdate("update membersinfo set name:='" + memberNameStr + "', address:= '" + addressStr
						+ "', phone:= '" + phoneStr + "' where phone = '" + phoneStr + "';");
				for (members obj : members.list) {
				...
				System.out.printf("이름: %s 주소: %s --> 이름: %s 주소: %s 로 수정되었습니다", name, address, memberNameStr,
						addressStr);
				return;
			}
			String queryLang = "insert into membersinfo (name, address, phone) values('" + memberNameStr + "', '"
					+ addressStr + "', '" + phoneStr + "');";
			int rowNum = stmt.executeUpdate(queryLang);
			System.out.println("회원등록 완료");
			members.list.add(new members(memberNameStr, addressStr, phoneStr, null, null, null)); // 자료구조에 회원 저장
		} catch (ClassNotFoundException cnfe) {
			System.out.println("해당 클래스를 찾을 수 없습니다." + cnfe.getMessage());
		} catch (SQLException se) {
			System.out.println(se.getMessage());
	...
	public static void memberDelete_DB(String phoneStr) { // DB에 저장되어 있는 회원정보 삭제 메소드
		...
		try {
			...
			for (int cnt = 0; cnt < members.list.size(); cnt++) { // 자료구조에 저장되어 있는 회원 데이터 삭제
				members obj = members.list.get(cnt);
				if (obj.phone.equals(phoneStr)) {
					members.list.remove(cnt);
					break;
				}
			}
		} catch (ClassNotFoundException cnfe) {
	...
	public static void printTable(JTable table) { // 프로그램 실행시 테이블 초기화 작업 메소드
		...
		try {
			...
			while (rs.next()) {
				members.list.add(new members(rs.getString("name"), rs.getString("address"), rs.getString("phone"),
						rs.getString("rentbook1"), rs.getString("rentbook2"), rs.getString("rentbook3")));
			}
		} catch (ClassNotFoundException cnfe) {
			System.out.println("해당 클래스를 찾을 수 없습니다." + cnfe.getMessage());
		} catch (SQLException se) {
	...
	public static void rental_DB(String selectBook[], String selectMember[]) { // 대여처리하는 메소드
		...
		try {
			...
			ResultSet rs = stmt.executeQuery(
					"select rentbook1, rentbook2, rentbook3 from membersinfo where phone = '" + selectMember[2] + "';");
			...
			if (rentbook1 == null) {
				stmt.executeUpdate("update membersinfo set rentbook1:= '" + selectBook[0] + "(" + selectBook[3]
						+ ")' where phone = '" + selectMember[2] + "';");
			} else if (rentbook2 == null) {
				stmt.executeUpdate("update membersinfo set rentbook2:= '" + selectBook[0] + "(" + selectBook[3]
						+ ")' where phone = '" + selectMember[2] + "';");
			} else {
				stmt.executeUpdate("update membersinfo set rentbook3:= '" + selectBook[0] + "(" + selectBook[3]
						+ ")' where phone = '" + selectMember[2] + "';");
			}
			...
			for (members obj : members.list) { // 자료구조에 있는 회원데이터에 대여한 책 추가하는 메소드
	...
	public static void returnBook_DB(String code) { // 반납 처리하는 메소드
		...
		try {
			...
			rs = stmt.executeQuery(
					"select name, phone, rentbook1, rentbook2, rentbook3 from membersinfo where rentbook1 like '%("
							+ code + ")' or " + "rentbook2 like '%(" + code + ")' or " + "rentbook3 like '%(" + code
							+ ")';");
			String phone = null;
			String name = null;
			String rentbook1 = "null";
			...
			for (members obj : members.list) { // 자료구조에 있는 회원데이터에서 반납하는 회원의 3개의 대여목록에서 반납도서 삭제
